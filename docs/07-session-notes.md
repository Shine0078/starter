...
```bash
npm install
npm test          # 388 tests, no database needed
npm run test:db   # 489 tests, spins up a real PostgreSQL and tears it down
...
| Check | Result |
|---|---|
| API tests, in-memory | 388 passed |
| API tests, real Postgres | 489 passed |
| `tsc --noEmit`, `npm run build` | clean |
| `flutter analyze`, `flutter test` | clean, 62 passed |

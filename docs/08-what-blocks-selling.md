...
| Tests | 428 without a database, 530 against real PostgreSQL, 76 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 428/530/76 | fixed in `07-session-notes.md`, but the pattern will recur |

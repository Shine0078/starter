...
| Tests | 416 without a database, 517 against real PostgreSQL, 69 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 416/517/69 | fixed in `07-session-notes.md`, but the pattern will recur |

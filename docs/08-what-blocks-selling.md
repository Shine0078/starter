...
| 5.8 | **Mobile testing is still thin** | 63 widget/design tests cover auth protocol, persisted-session refresh, concurrent refresh, recovery, deletion, navigation, grouped transaction filters, offline cache, accessibility scaling, analytics visuals, reactive data invalidation, notification preferences, connectivity diagnostics, Plaid allowlist guidance, offline global logout, secure cleanup failure, and plan/paywall paths. No device integration or golden tests | 2 weeks |

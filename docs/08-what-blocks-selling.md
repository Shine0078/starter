...
| Tests | 425 without a database, 526 against real PostgreSQL, 74 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 425/526/74 | fixed in `07-session-notes.md`, but the pattern will recur |

...
| Tests | 385 without a database, 486 against real PostgreSQL, 59 Flutter tests, Android debug and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 385/486/59 | fixed in `07-session-notes.md`, but the pattern will recur |

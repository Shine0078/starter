...
| Tests | 392 without a database, 493 against real PostgreSQL, 65 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 392/493/65 | fixed in `07-session-notes.md`, but the pattern will recur |

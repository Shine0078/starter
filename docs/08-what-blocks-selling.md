...
| Tests | 417 without a database, 518 against real PostgreSQL, 71 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 417/518/71 | fixed in `07-session-notes.md`, but the pattern will recur |

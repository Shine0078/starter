...
| Tests | 407 without a database, 508 against real PostgreSQL, 68 Flutter tests, Android release and web release builds; all passing |
...
| 9.4 | Test counts drift out of date and are then quoted as evidence — older counts were stale until re-measured at 407/508/68 | fixed in `07-session-notes.md`, but the pattern will recur |

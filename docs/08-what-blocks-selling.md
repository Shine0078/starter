...
| Tests | 487 without a database, 596 against real PostgreSQL, 94 Flutter tests, Android release and web release builds; all passing (re-measured 2026-08-15) |
...
| 2.3 | **Reauth / broken-link handling** | Completed: `ITEM_LOGIN_REQUIRED` becomes a visible reconnect state, Link update mode reuses healthy Items, and revoked Items start a fresh Link session after deletion recovery | production institution testing remains |
...
| 2.5 | **Merchant lexicon is tiny** | Categorisation quality on real data is unknown. The top ~2,000 merchants cover most volume; the lexicon now covers the major US/CA supermarkets, restaurants, streaming/gaming/software subscriptions, fuel, insurers, telecoms, utilities, and airlines, but real-market coverage remains partial | 1–2 weeks + ongoing |
...
| 4.7 | **External error tracking optional** | Structured logs omit financial payloads. `SENTRY_DSN` is accepted as an optional HTTPS sink and the mobile app stores a redacted local crash log. A production Sentry (or equivalent) project is still an owner account | external account + days |
...
| # | Item | Detail | Effort |
|---|---|---|---|
| 5.1 | **Primary navigation aligned with MISSION2** | Five destinations now stay focused on Home, Transactions, Analytics, Accounts, and Profile. Budgets, goals, planning, the financial calendar, subscriptions, notifications, and settings remain one tap away in the Profile hub instead of competing for permanent tab space | 2–3 weeks for remaining breadth/polish |
| 5.2 | **Offline cache, idempotent preference queue, and scheduled refresh implemented** | Authenticated GET responses fall back to a 30-day, user-scoped encrypted cache; transaction preference edits queue offline, collapse to the latest value, replay on resume, and show pending/stale state. Android now has a network-constrained Workmanager refresh; iOS declares the matching BGTask and plugin registrant. Bank balances remain server-authoritative and iOS timing is best-effort | physical Android/iPhone validation + broader conflict UX |
| 5.3 | **No state management** | `setState` only. Honest at one screen, unworkable at twenty | with 5.1 |
| 5.4 | **Remote push delivery and local alerts implemented** | Persistent preferences and the mobile centre cover budgets, utilization, low balances, bank sync, upcoming bills, subscription price rises, possible duplicates, and conservative spending outliers. FCM HTTP v1 is now an opt-in server adapter, with APNs routed by Firebase; notifications are persisted first and lock-screen payloads disclose no financial details | Firebase/Apple credentials, client token registration, and physical-device validation |
...
| 5.8 | **Mobile testing is still thin** | 94 widget/design tests cover auth protocol, persisted-session refresh, concurrent refresh, keystore write/cleanup failure, recovery, deletion, navigation, grouped transaction filters, offline cache, accessibility scaling, analytics visuals, reactive data invalidation, notification preferences, connectivity diagnostics, Plaid allowlist guidance, offline global logout, plan/paywall paths, assistant response parsing, secure-session persistence error messaging, historical trend chart, financial-calendar events and low-balance warnings, primary navigation/profile routing, categorization-rule parsing, keystore-locked logout recovery, stale-token cached reads during refresh outages, user-friendly error mapping, and the French dashboard dialog regression added 2026-08-15. No device integration or golden tests | 2 weeks |
...
| 5.11 | **Crash reporting is local-only** | The app stores redacted crash summaries on-device. A production crash-free rate still requires an owner-configured provider and physical-device evidence | owner DSN + days |
...
- Custom automation rules, life timeline

...
| On device | Refresh/access tokens, app-lock preference, and the offline-cache key use Keychain / Android Keystore. Cached financial payloads use AES-256-GCM with authenticated context, a 30-day ceiling, user scoping, purge-on-sign-out, and an optional system-authentication app gate. A non-sensitive signed-out marker also lives in platform preferences so logout survives a temporarily locked keystore | Consider whole-file SQLCipher if the threat model requires hiding cache metadata as well as payloads; verify app-lock behavior on physical hardware |
